  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-slate-900">Kelola Editor &amp; Leader</h1>
          <p class="text-slate-400 text-xs mt-0.5">Akun editor & pimpinan memiliki akses lintas OPD</p>
        </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="user in users" :key="user.id" class="border-b border-slate-50 hover:bg-slate-50">
              ...
              <td class="py-3 px-2 text-slate-500">{{ user.opd?.name ?? 'Lintas OPD' }}</td>
            ...
            <tr v-if="!users.length">
              <td colspan="5" class="py-6 text-center text-slate-400">Belum ada akun editor/leader.</td>
            </tr>
          </tbody>
        </table>
      </div>
