    ...
    <div class="space-y-6">
      <div class="space-y-3">
        ...
        <div>
          ...
          <p class="text-slate-400 text-xs mt-0.5">Kelola akun kontributor seluruh OPD</p>
        </div>
      </div>
        ...
        <table class="min-w-full text-sm" v-if="contributors.length">
          <thead class="bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">Akses OPD</th>
            ...
            <tr v-for="user in contributors" :key="user.id" class="hover:bg-slate-50 transition">
              ...
              <td class="px-5 py-3.5 text-slate-500 text-xs">
                <div v-if="user.accessible_opds?.length" class="flex flex-wrap gap-1">
                  <span v-for="opd in user.accessible_opds" :key="`${user.id}-${opd.id}`" class="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">
                    {{ opd.name }}
                  </span>
                </div>
                <span v-else>{{ user.opd?.name ?? '-' }}</span>
              </td>
              <td class="px-5 py-3.5">
              ...
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                  <Link :href="route(`${routeBase}.edit`, user.id)" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium">Edit</Link>
...
function applyFilter() {
  router.get(route(routeBase), selectedOpd.value ? { opd_id: selectedOpd.value } : {}, {
    preserveState: true,
    replace: true,
  });
}
...
function deleteUser(id) {
  if (confirm('Yakin ingin menghapus akun ini?')) {
    router.delete(route(`${routeBase}.destroy`, id));
  }
}
</script>
