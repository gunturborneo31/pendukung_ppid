  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-bold text-slate-900">Berita Disetujui Editor</h1>
          <p class="text-slate-400 text-xs mt-0.5">Daftar berita yang sudah Anda setujui dan dipublikasikan</p>
        </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div v-else class="py-16 text-center">
          ...
          <p class="text-slate-500 text-sm">Belum ada berita disetujui oleh Anda.</p>
        </div>
      </div>
    </div>
  </AppLayout>
...
function applyFilter() {
  router.get('/editor/approved', selectedOpd.value ? { opd_id: selectedOpd.value } : {}, {
    preserveState: true,
    replace: true,
  });
}
