        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span class="text-xs" :class="seoTitleLen > 60 ? 'text-red-500' : 'text-gray-400'">{{ seoTitleLen
              }}/60</span>
            </div>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <span class="text-xs"
                :class="seoDescLen > 160 ? 'text-red-500' : seoDescLen > 120 ? 'text-yellow-500' : 'text-green-500'">{{
                  seoDescLen }}/160</span>
            </div>
            <div>
                  ...
                  <div>
                    ...
                    <div v-if="form.ig_media.length" class="mt-3 space-y-2">
                      <div v-for="(file, idx) in form.ig_media" :key="`${file.name}-${idx}`"
                        ...
                        <div
                          class="w-16 h-16 flex-shrink-0 flex items-center justify-center bg-white border rounded overflow-hidden">
                          <img v-if="isImage(file)" :src="getFileUrl(file)" class="object-cover w-full h-full" />
                          <video v-else-if="isVideo(file)" :src="getFileUrl(file)" class="object-cover w-full h-full"
                            controls />
                        ...
                        <div class="flex gap-1">
                          <button v-if="idx > 0" type="button" @click="moveMediaUp(idx)"
                            class="text-blue-500 hover:text-blue-700 text-xs font-medium px-1.5 py-0.5 rounded hover:bg-blue-100" title="Naik">
                            ▲
                          </button>
                          <button v-if="idx < form.ig_media.length - 1" type="button" @click="moveMediaDown(idx)"
                            class="text-blue-500 hover:text-blue-700 text-xs font-medium px-1.5 py-0.5 rounded hover:bg-blue-100" title="Turun">
                            ▼
                          </button>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
          <div class="flex items-center justify-between gap-4">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">Tambahkan dokumen, foto, atau video pendukung di bawah konten
                artikel.</p>
            </div>
            <label
          ...
          <div v-if="supportingFiles.length" class="space-y-3">
            <div v-for="(file, idx) in supportingFiles" :key="`${file.name}-${idx}`"
              ...
              <div class="flex flex-col lg:flex-row gap-4">
                <div
                  class="w-full lg:w-44 h-36 bg-white border rounded-lg overflow-hidden flex items-center justify-center">
                  <img v-if="isImage(file)" :src="getFileUrl(file)" class="object-cover w-full h-full" />
                  <video v-else-if="isVideo(file)" :src="getFileUrl(file)" class="object-cover w-full h-full"
                    controls />
                ...
                <div class="flex-1 space-y-3">
                  ...
                  <div class="flex items-center gap-3 text-xs">
                    <button type="button" @click="editSupportingFile(idx)" class="text-blue-600 hover:underline">Ubah
                      File</button>
                    <button type="button" @click="removeSupportingFile(idx)"
...
const seoChecks = computed(() => [
  ...
  { label: 'Thumbnail diunggah', pass: !!form.thumbnail, param: form.thumbnail ? 'Ada' : 'Kosong' },
